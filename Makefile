...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live Fastmail)"
...
test-e2e:
	FASTMAIL_E2E=1 uv run pytest tests/e2e/ -v -m e2e

test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/fastmail_blade_mcp --cov-report=term-missing -v
...
type-check:
	uv run mypy src/fastmail_blade_mcp
